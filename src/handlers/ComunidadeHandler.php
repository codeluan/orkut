...
use \src\models\ComunidadeRelation;
use \src\models\User;
...
class ComunidadeHandler {
    ...
    public static function idExists($id) {
     ...
     public static function getComunidades($atts) {
        ...
        if($data) {
            ...
            $comunidades->somarelacionadas = $data['somarelacionadas'];
        ...
        //gerar as relacionadas das comunidades
        $relacionadas = ComunidadeRelation::select()->where('comunidade_from', $atts)->get();
        foreach($relacionadas as $relacionada) {
            $relacionadaData = Comunidade::select()->where('id', $relacionada['comunidade_to'])->one();

            $newRelacionada = new Comunidade();
            $newRelacionada->id = $relacionadaData['id'];
            $newRelacionada->nome = $relacionadaData['nome'];
            $newRelacionada->avatar = $relacionadaData['avatar'];
            $newRelacionada->somamembros = $relacionadaData['somamembros'];

            $comunidades->relacionadas[] = $newRelacionada;
    ...
    public static function addComunidade($idUser, $nome, $descricao) {
        ...
        if(!empty($idUser) && !empty($nome)) {
            ...
            // buscar total de comunidades que o usuario é dono
        $total = Comunidade::select()
        ->where('id_usuario', $idUser)
        ->count();
        // retorna o resultado

            // inserir quantidade de comunidades que é dono
        User::update([
        'somarcomunidadesdono' => $total
        ])
        ->where('id', $idUser)
        ->execute();
        }
    ...
    public static function addTopico($idUser, $idcmm, $assunto, $mensagem) {
        ...
        if(!empty($idUser) && !empty($assunto)) {
            ...
            // buscar total de topicos por comunidade
        $total = ComunidadeTopico::select()
    ...
    public static function addMensagemTopico($idUser, $idcmm, $idtopic, $assunto, $mensagem) {
        ...
        if(!empty($idUser) && !empty($assunto)) {
            ...
            // buscar total de mensagens por topico
        $total = ComunidadeMensagen::select()
    ...
    // EDITAR A COMUNIDADE
    public static function updateComunidade($comunidade, $nome, $descricao, $cidade, $estado, $cep, $pais, $idioma, $categoria, $tipo, $privacidade, $avatarName){
    ...
    // GERANDO OS MEMBROS DAS COMUNIDADE E RETORNANDO MEMBROS
    public static function _membrosListToObject($membroList , $loggedUserId) {
        $membros = [];
            foreach($membroList as $user) {
                $userData = User::select()->where('id', $user['id_usuario'])->one();
        ...
        // GERANDO MEMBROS E PAGINAÇÃO DAS COMUNIDADES
    public static function getComunidadeMembros($comunidade, $page, $loggedUserId) {
        $perPage = 14; // DEFINIR QUANTIDADE DE MEMBROS POR PÁGINA

        // .1 pegar membros na comunidade
        $membroList = ComunidadeMembro::select()
        ->where('id_comunidade', $comunidade)
        ->page($page, $perPage)
        ->get();
        ...
        $membros = self::_membrosListToObject($membroList, $loggedUserId);
    ...
    // GERANDO AS COMUNIDADES RELACIONADAS
    public static function _relacionadasListToObject($relacionadasList) {
        $relacionadas = [];
            foreach($relacionadasList as $user) {
                $comunidadeData = Comunidade::select()->where('id', $user['comunidade_to'])->one();

                $newRelacionada = new Comunidade();
                $newRelacionada->id = $comunidadeData['id'];
                $newRelacionada->nome = $comunidadeData['nome'];
                $newRelacionada->avatar = $comunidadeData['avatar'];
                $newRelacionada->somamembros = $comunidadeData['somamembros'];

            $relacionadas[] = $newRelacionada;
        }
        return $relacionadas;
    }
        // GERANDO AS COMUNIDADES RELACIONADAS E PAGINAÇÃO DAS COMUNIDADES
    public static function getComunidadeRelacionadas($comunidade, $page) {
        $perPage = 14; // DEFINIR QUANTIDADE DE MEMBROS POR PÁGINA

        // .1 pegar comunidades relacionadas
        $relacionadasList = ComunidadeRelation::select()
        ->where('comunidade_from', $comunidade)
        ->page($page, $perPage)
        ->get();

        // 1.1 total de página - gerar paginação
        $total = ComunidadeRelation::select()
        ->where('comunidade_from', $comunidade)
        ->count();
        $pageCount = ceil($total / $perPage);

        // 1.2 transformas os resultados em objetos dos models ( gerar as comunidades relacionadas)
        $relacionadas = self::_relacionadasListToObject($relacionadasList);
    ...
    // Adicionar e remover Comunidades Relacionadas
    public static function comunidadeIsRelacionar($comunidade, $comunidadeRelacionada) {
        $datas = ComunidadeRelation::select()
            ->where('comunidade_from', $comunidade)
            ->where('comunidade_to', $comunidadeRelacionada)
        ->one();
    ...
    // FUNÇÃO INSERIR Comunidades Relacionadas no db
    public static function comunidadeRelacionarOn($comunidade, $comunidadeRelacionada) {
        ComunidadeRelation::insert([
            'comunidade_from' => $comunidade,
            'comunidade_to' => $comunidadeRelacionada
        ])->execute();

         // buscar total de membros por usuario
         $totalRelacionadas = ComunidadeRelation::select()
         ->where('comunidade_from', $comunidade)
         ->count();
         // retorna o resultado
         ...
         // inserir quantidade de membros na coluna somamembros da comunidade
         Comunidade::update([
         'somarelacionadas' => $totalRelacionadas
         ])
         ->where('id', $comunidade)
         ->execute();
        ...
        // FUNÇÃO REMOVER Comunidades Relacionadas NO DB
    public static function comunidadeRelacionarOff($comunidade, $comunidadeRelacionada) {
        ComunidadeRelation::delete()
            ->where('comunidade_from', $comunidade)
            ->where('comunidade_to', $comunidadeRelacionada)
        ->execute();

        // buscar total de Comunidades Relacionadas
        $totalRelacionadas = ComunidadeRelation::select()
        ->where('comunidade_from', $comunidade)
        ->count();
        // retorna o resultado

        // inserir quantidade de Comunidades Relacionadas na coluna somarelacionadas
        Comunidade::update([
        'somarelacionadas' => $totalRelacionadas
        ])
        ->where('id', $comunidade)
